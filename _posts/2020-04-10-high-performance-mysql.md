...
+ [高性能MySQL.第3版.pdf](https://li-qr.github.io/assets/pdf/%E9%AB%98%E6%80%A7%E8%83%BDMySQL.%E7%AC%AC3%E7%89%88.pdf)

## RedoLog、UndoLog、MVCC

| 组件 | 保障的ACID特性 | 核心作用 | 关键原理 |
| :--- | :--- | :--- | :--- |
| **Redo Log (重做日志)** | 持久性 (D) | 确保已提交事务的数据在宕机后不丢失。 | **WAL (预写日志)**：先写日志，再写磁盘。顺序写入，性能高。是**物理日志**，记录页级别的修改。 |
| **Undo Log (回滚日志)** | 原子性 (A) | 用于事务回滚，并为MVCC提供数据版本。 | **逻辑日志**，记录修改前的数据（反向操作）。通过隐藏列 `DB_ROLL_PTR` 串联成**版本链**。 |
| **MVCC (多版本并发控制)** | 隔离性 (I) | 实现无锁并发读，读写不互相阻塞。 | 利用 **Read View (读视图)** 和 Undo Log 版本链，判断数据版本的可见性。在 `REPEATABLE READ` 级别下，事务内复用同一个 Read View。 |


### Undo Log 存储结构详解

#### 物理存储
- **MySQL 5.6+**：可存储于独立的 **Undo 表空间**（`undo_001`, `undo_002`）。
- **MySQL 8.0**：默认独立存储，支持**自动截断 (Truncate)** 以回收空间。

#### 逻辑层次（自上而下）
```text
Undo 表空间 (Undo Tablespace)
    └── 回滚段 (Rollback Segment) - 最多128个，支持高并发
            └── Undo 段 (Undo Segment) - 分为 Insert Undo 和 Update Undo 两类
                    └── Undo 页 (Undo Page) - 16KB，存储具体的Undo记录
```

#### 生命周期管理
- **Insert Undo**：事务提交后即可被标记清理。
- **Update Undo**：需等待所有可能读取该历史版本的事务结束后，由后台 **Purge 线程** 统一清理。


### MVCC 可见性判断（Read View 规则）

**判断核心**：数据行的 `DB_TRX_ID` (最后修改该行的事务ID) 与当前 Read View 的对比。

| 条件 | 判断结果 |
| :--- | :--- |
| `DB_TRX_ID == creator_trx_id` | **可见** (当前事务自己的修改) |
| `DB_TRX_ID < min_trx_id` | **可见** (修改事务在Read View生成前已提交) |
| `DB_TRX_ID >= max_trx_id` | **不可见** (修改事务在Read View生成后才开始) |
| `min_trx_id <= DB_TRX_ID < max_trx_id` 且在 `m_ids` 列表中 | **不可见** (修改事务在Read View生成时未提交) |
| `min_trx_id <= DB_TRX_ID < max_trx_id` 且不在 `m_ids` 列表中 | **可见** (修改事务在Read View生成前已提交) |


### 复杂场景下的锁与回滚机制

#### 场景一：锁阻塞优先于快照读
- 当 T1 执行 `UPDATE` 并持有 **X 锁 (排他锁)** 时，T2 的 `UPDATE` 会**被阻塞**。
- T2 在获得锁之前，既无法读取最新数据，也无法生成自己的 Undo Log。
- **回滚时**：T1 回滚会利用 Undo Log **物理恢复**数据，将 `DB_TRX_ID` 和 `DB_ROLL_PTR` 恢复原状。

#### 场景二：快照读 (SELECT) vs 当前读 (UPDATE)
- **`SELECT` 是快照读**，基于 Read View 读取历史版本。
- **`UPDATE` 是当前读**，总是读取数据页上的最新已提交数据。
- **影响**：在 RR 级别下，`UPDATE` 天然解决了“丢失更新”问题——它不会基于事务自身之前快照读的值去修改。

#### 场景三：死锁回滚
- 死锁发生时，MySQL 会选择回滚**代价较小**的事务。
- 被选中的牺牲者（如 T1）会利用其 Undo Log **完整回滚所有修改**，并释放持有的所有锁，仿佛该事务从未执行过。


###  频繁回滚产生的“空洞”与碎片

#### 自增主键空洞
- **核心机制**：自增ID一旦申请，**不会因回滚而退回**。
- **后果**：导致主键值不连续，且被“浪费”的ID**无法被重新使用**。

#### 数据页空间碎片
- **产生原因**：回滚的事务已经分配了数据页（16KB），回滚后该页被标记为“空闲”，但文件空间**不会归还给操作系统**。
- **后果**：表空间文件 (`.ibd`) 只增不减，占用虚高。在自增主键下，新数据通常插入文件末尾，导致文件中间的“空洞”**永久无法被新数据复用**。

#### 清理与回收方法
| 问题 | 治理方案 |
| :--- | :--- |
| **Undo 表空间膨胀** | - **MySQL 8.0**：启用 `innodb_undo_log_truncate` 自动截断。<br>- **MySQL 5.7**：手动切换并删除非活跃的 Undo 表空间。 |
| **表数据碎片** | 执行 `ALTER TABLE table_name ENGINE=InnoDB;` 或 `OPTIMIZE TABLE table_name;` 重建表，重组数据。**注意**：此操作会锁表。 |
...
| 参数 | 作用 | 建议 |
| :--- | :--- | :--- |
| `innodb_flush_log_at_trx_commit` | Redo Log 刷盘策略。<br>`1`：每次提交刷盘（最安全）；<br>`0`：每秒刷盘（性能最高）；<br>`2`：写入OS缓存（性能与安全的折中）。 | 生产环境推荐 `1`，保证数据不丢失。 |
| `innodb_max_undo_log_size` | Undo 表空间的最大大小 (默认1GB)。 | 超过此值会触发自动截断 (需开启 `innodb_undo_log_truncate`)。 |
| `innodb_rollback_segments` | 实例可使用的回滚段数量 (默认128)。 | 影响最大并发事务数，通常保持默认。 |


## MySQL InnoDB 刷盘机制、组提交与崩溃恢复

### 刷盘机制：Redo Log 与 脏数据页

#### 核心认知
**“刷写磁盘”在InnoDB中是泛指，包含两个完全独立且同等重要的动作：**

| 刷盘动作 | 内存来源 | 磁盘目标 | 写入类型 | 触发时机 |
| :--- | :--- | :--- | :--- | :--- |
| **刷 Redo Log** | Log Buffer（日志缓冲区） | `ib_logfile` 文件 | **顺序写**（快） | 事务提交时（由 `innodb_flush_log_at_trx_commit` 控制） |
| **刷 脏数据页** | Buffer Pool（缓冲池）中被修改的数据页 | `.ibd` 数据文件 | **随机写**（慢） | 异步触发（Checkpoint、Buffer Pool不足、后台线程定期刷） |

#### WAL（预写日志）技术
> **Redo Log 必须 100% 保证在脏数据页被刷入磁盘之前，率先持久化到磁盘。**

- **宕机恢复**：如果脏页未刷盘，MySQL重启后会读取Redo Log，将丢失的修改重做回来。
- **性能原理**：利用顺序写Redo Log代替随机写数据页，极大提升事务提交效率。

#### 关键参数：`innodb_flush_log_at_trx_commit`

| 值 | 含义 | 安全性 | 性能 |
| :--- | :--- | :--- | :--- |
| **1（默认）** | 每次事务提交立即刷盘 | **最高**（不丢数据） | 最低 |
| **2** | 写入OS缓存，由操作系统决定落盘 | 折中（宕机可能丢1秒数据） | 较高 |
| **0** | 每秒刷一次 | 最低（宕机可能丢1秒数据） | **最高** |

**生产建议**：核心交易库使用 `1`；日志、监控等可容忍丢数据的业务使用 `2` 或 `0`。


### 合并提交（Group Commit）

#### 什么是合并提交？
**将多个事务的提交刷盘操作“攒”成一批，合并成一次磁盘I/O操作来执行。** 在MySQL 5.6+中特指 **Binary Log Group Commit (BLGC)**。

#### 痛点背景
开启双1配置（`sync_binlog=1` 且 `innodb_flush_log_at_trx_commit=1`）时，每个事务提交需要执行2次`fsync`（刷Redo Log + 刷Binlog）。高并发下磁盘I/O成为瓶颈，TPS急剧下降。

#### 核心原理：三个阶段流水线

| 阶段 | 操作 | 锁/互斥量 | 核心优化 |
| :--- | :--- | :--- | :--- |
| **Flush（刷入OS缓存）** | 将多个事务的Binlog和Redo Log从内存缓冲区写入操作系统文件缓存 | `LOCK_log` | 连续写入，减少锁争用 |
| **Sync（同步到磁盘）** | **仅执行1次`fsync`**，将这批日志强制落盘 | `LOCK_sync` | **N个事务，1次磁盘I/O** — 性能提升的根本 |
| **Commit（提交完成）** | 逐个更新事务提交状态（GTID、存储引擎层标记等） | `LOCK_commit` | 纯内存操作，速度极快 |

#### 控制参数

| 参数 | 作用 | 生产建议 |
| :--- | :--- | :--- |
| `binlog_group_commit_sync_delay` | 等待多少微秒再执行Sync，让更多事务加入当前组。**值越大，吞吐越高，但延迟也越高**。 | 默认为0。高并发写入场景可设为 **10000（10ms）**。 |
| `binlog_group_commit_sync_no_delay_count` | 若排队事务数达到此阈值，立即触发Sync，不再等待延迟时间。 | 默认为0。可设为 **50~100**，防止等待过长。 |

#### 实战洞察
> **用少量延迟换海量吞吐**：调整 `binlog_group_commit_sync_delay` 到10ms，QPS可从800提升到5000，而RT只增加不到5ms。


###  故障恢复：如何确定Redo Log回溯位置

#### 核心概念：LSN（Log Sequence Number，日志序列号）
- 一个**单调递增的64位数字**，唯一标识每一条Redo Log记录。
- **每个数据页头部也存有LSN**，表示该页最后一次修改时对应的日志序列号。
- 通过对比日志LSN和页面LSN，即可判断哪些修改已落盘、哪些尚未。

#### 回溯的“锚点”：Checkpoint（检查点）
- **作用**：将内存中的部分脏页强制刷盘，并**记录此次操作对应的LSN**到Redo Log文件的固定位置。
- **信息存储**：MySQL会将Checkpoint信息**交替**保存在`ib_logfile`开头，通过递增的`checkpoint_no`编号来识别最新的一个。

#### 恢复流程（4步）

1. **定位Checkpoint**：读取Redo Log文件固定位置，解析出最新的Checkpoint信息。
2. **获取起始LSN**：从Checkpoint中提取出 `last_checkpoint_lsn`，这就是恢复的**起点**。
3. **从起点开始应用日志**：从 `last_checkpoint_lsn` 向后扫描所有Redo Log记录，并应用到对应的数据页。
4. **判断终点**：扫描到Redo Log文件末尾（或不完整的日志块）时，恢复结束。

#### 智能跳过已落盘的日志
在应用每条Redo Log前，InnoDB会做判断：
- 读取目标数据页头部的LSN。
- 如果 **数据页的LSN >= 待应用日志的LSN**，说明该页的修改比这条日志更新，则**跳过该日志**。
- 这个机制保证了恢复过程的**幂等性**，避免重复应用导致数据损坏。

#### 关键参数
| 参数 | 作用 | 说明 |
| :--- | :--- | :--- |
| `innodb_log_checkpoint_now` | 手动触发Checkpoint | 通常由系统自动执行，无需手动干预 |
| `innodb_checkpoint_disabled` | 禁用自动Checkpoint（调试用） | **生产环境绝不能开启** |


### 全貌总结：一条事务提交后，数据到底经历了什么？

为了帮你打通所有知识点，这里给出一个完整的闭环流程：

```text
事务提交
    ↓
1. Redo Log 写入 Log Buffer（内存）
2. Binlog 写入 Binlog Cache（内存）
    ↓
3. 【Flush阶段】写入 OS Cache（文件缓存）
    ↓
4. 【Sync阶段】执行 1 次 fsync，将 Redo Log + Binlog 持久化到磁盘
    ↓
5. 【Commit阶段】更新事务状态，返回客户端“提交成功”
    ↓
6. 【异步后台刷脏页】Buffer Pool 中的脏页在 Checkpoint 时被异步刷入 .ibd 文件
    ↓
7. 【崩溃恢复】若步骤4完成、步骤6未完成时宕机 → 通过 Redo Log 从 Checkpoint 处重做恢复
```
...
### 索引的本质与代价
- **本质**：排好序的、用于快速查找的数据结构（相当于书的目录）。
- **核心代价**：**空间换时间**。占用磁盘空间，同时拖慢写入速度（`INSERT`/`UPDATE`/`DELETE`需同步维护索引树）。


### 底层数据结构：为什么必须是B+树？

| 数据结构 | 为什么被Pass | B+树的胜出点 |
| :--- | :--- | :--- |
| **Hash** | 等值查询O(1)，但**不支持范围查询和排序**。 | 1. **树高矮**（2~4层）：非叶子节点不存数据，只存键值+指针，一页能存上千个索引项，磁盘I/O次数极少。<br>2. **叶子节点有序双向链表**：天然支持 `>`、`<`、`ORDER BY`。<br>3. **稳定I/O**：所有数据都在叶子节点，每次查询I/O次数固定。 |
| **二叉树** | 自增主键下退化为链表，树高极高。 | |
| **B树** | 非叶子节点也存数据，导致页内索引项变少，树变高。 | |
...
| 类型 | 叶子节点存什么 | 关键结论 |
| :--- | :--- | :--- |
| **聚簇索引（主键索引）** | **整行数据** | 1. InnoDB中即主键。<br>2. **一个表只有一个**。<br>3. 数据物理顺序与索引逻辑顺序一致。 |
| **二级索引（非聚簇索引）** | **主键ID** | 1. 一个表可以有多个。<br>2. 查询流程：二级索引 → 主键ID → 回表（回到聚簇索引查整行）。<br>**优化目标**：尽量用覆盖索引避免回表。 |


### 联合索引与最左前缀法则（决定“查找路径”）

#### 核心规则
> **索引从左到右匹配，遇到范围查询（`>`、`<`、`BETWEEN`、`LIKE`）即停止匹配后续列。**

#### 经典案例分析（索引 `(a, b, c)`）

| SQL 条件 | 索引命中情况 | 底层原因 |
| :--- | :--- | :--- |
| `a=1 AND b=2 AND c=3` | ✅ 命中全部 | 等值匹配，完美利用。 |
| `a=1 AND c=3` | ⚠️ 只命中 `a` | 跳过了 `b`，B+树无法在分支上继续比较 `c`。 |
| `b=2 AND c=3` | ❌ **完全失效** | 未命中最左列 `a`，无法定位根节点分支。 |
| `a=1 AND b>2 AND c=3` | ⚠️ 命中 `a` 和 `b` | `b` 是范围查询，`c` 停止匹配（无法用于缩小范围）。 |

#### 优化口诀
> **等值查询放最左，范围排序放后面，覆盖索引带全部。**


### 索引下推（ICP，Index Condition Pushdown）—— 5.6+ 重大优化

#### 关键认知（解决你之前的疑惑）
**最左前缀和ICP并不矛盾，它们作用于B+树检索的两个不同阶段：**

| 机制 | 作用的阶段 | 核心职责 | 比喻（图书馆） |
| :--- | :--- | :--- | :--- |
| **最左前缀** | **定位起点（Search）** | 决定从B+树根节点往下走到哪个叶子节点位置。 | 必须根据“省份→城市→区”找到对应的书架。 |
| **索引下推（ICP）** | **遍历过滤（Filter）** | 在叶子节点链表扫描时，提前用索引列值筛掉不合格记录，减少回表。 | 在书架前直接翻看，把不符合“页码”的书当场扔回去，不搬回前台。 |
...
```
- **无ICP**：找出所有姓“张”的ID（假设10万个）→ 回表10万次 → 再过滤 `age=20`。**I/O爆炸**。
- **有ICP（默认开启）**：
  1. **定位**：根据最左前缀 `name LIKE '张%'` 定位到叶子节点起点。
  2. **遍历+过滤**：在扫描索引链表时，**直接读取当前叶子节点上的 `age` 列值**（因为`age`也在索引中）。
  3. **决策**：若 `age != 20`，直接跳过，**不回表**；只有 `age=20` 的（假设100个）才回表。
  4. **结果**：回表次数从10万次**骤降至100次**。

> **一句话总结**：`age`列虽然因最左前缀无法帮助定位起点，但一旦开始扫描，ICP就能利用索引页里的`age`值进行过滤。**一个管“怎么走”，一个管“走的时候怎么扔”。**

#### ICP vs 覆盖索引（面试混淆点）

| 优化手段 | `EXPLAIN` 的 `Extra` | 本质区别 |
| :--- | :--- | :--- |
| **覆盖索引** | `Using index` | 查询的所有字段都在索引中，**完全不需要回表**。 |
| **索引下推（ICP）** | `Using index condition` | 依然需要回表（如`SELECT *`），但**回表次数被大幅减少**。 |
...
| 失效场景 | 错误示例 | 底层原因 |
| :--- | :--- | :--- |
| **对索引列用函数** | `WHERE YEAR(date) = 2024` | 必须对每行计算，无法利用索引树比较。 |
| **隐式类型转换** | `WHERE phone = 13800138000`（phone是VARCHAR） | MySQL隐式调用了 `CAST(phone AS INT)`，等同用了函数。 |
| **左模糊查询** | `WHERE name LIKE '%三'` | B+树从左向右匹配，前缀`%`让树无法定位起点。 |
| **OR连接非索引列** | `WHERE a=1 OR b=2`（b无索引） | 必须全表扫描。 |
| **!= 或 <>** | `WHERE status != 1` | 结果集过大，优化器认为全表扫描更便宜。 |
| **联合索引不满足最左** | `WHERE b=1`（索引`(a,b)`） | 无法定位根节点分支。 |
...
| 字段 | 关注点 | 解读 |
| :--- | :--- | :--- |
| **`type`** | 至少达到 **`range`**，拒绝 `ALL` | 性能排序：`system > const > eq_ref > ref > range > index > ALL` |
| **`Extra`** | `Using index`（好）vs `Using filesort`（差）vs `Using temporary`（极差） | `filesort`代表额外文件排序，需建联合索引消除；`temporary`常见于`GROUP BY`不当。 |
| **`key_len`** | 计算实际用了索引多少字节 | 可判断联合索引用到哪几列。 |


### 索引维护与碎片治理

- **页分裂**：使用**UUID等随机主键**会导致频繁页分裂，插入性能极差。**强烈建议用自增ID**（顺序追加，不分裂）。
- **索引碎片**：频繁`UPDATE`变长字段和`DELETE`会产生碎片（填充率降低）。可定期在**业务低峰期**执行 `OPTIMIZE TABLE t;`（会锁表）。

好的，我把刚才讨论中涉及的 **锁粒度、锁类型、行锁算法、死锁检测与实战** 全部整合成一份完整笔记。这次补上了“前后间隙锁本质”、“RC主从不一致原理”、“死锁检测算法”和“四个条件具象化理解”，可以直接背。
...
| 粒度 | 引擎 | 特点 | 场景 |
| :--- | :--- | :--- | :--- |
| **表锁** | MyISAM / InnoDB | 开销小，并发低，写锁阻塞一切 | DDL、全表迁移 |
| **行锁** | **InnoDB 专属** | 开销大，并发高，**基于索引项加锁** | 高并发OLTP（订单、库存） |
| **页锁** | BDB（废弃） | 介于两者之间 | 忽略 |

> **铁律**：InnoDB行锁锁的是**索引项**。`UPDATE` 不走索引 → 行锁升级为表锁（锁全表），性能瞬间崩塌。


### 锁的类型（按读写意图）

#### S锁（共享锁/读锁） vs X锁（排他锁/写锁）

| 锁类型 | 别名 | 兼容性 | 加锁SQL | 释放 |
| :--- | :--- | :--- | :--- | :--- |
| **S锁** | 读锁 | S与S兼容，S与X互斥 | `SELECT ... LOCK IN SHARE MODE` | 事务结束 |
| **X锁** | 写锁 | X与任何锁都互斥 | `SELECT ... FOR UPDATE` / `UPDATE` / `DELETE` | 事务结束 |

#### 意向锁（IS / IX）—— 表级“旗帜锁”（面试必考）

- **本质**：表级锁，**InnoDB自动添加**，无需人工干预。
- **作用**：加表锁（如 `LOCK TABLES ... WRITE`）时，无需逐行遍历检查行锁，直接看意向锁即可判断。**极大提升加表锁效率**。

| 意向锁 | 含义 | 触发时机 |
| :--- | :--- | :--- |
| **IS（意向共享锁）** | 事务准备给某些行加S锁 | `SELECT ... LOCK IN SHARE MODE` 前自动加 |
| **IX（意向排他锁）** | 事务准备给某些行加X锁 | `UPDATE` / `SELECT ... FOR UPDATE` 前自动加 |

**兼容性矩阵（背诵口诀：IS/IX互相兼容，表级S/X堵死IX）**：
...
| 算法 | 锁定对象 | 触发条件 | 作用 |
| :--- | :--- | :--- | :--- |
| **Record Lock（记录锁）** | 精准锁定**单条索引记录** | 等值查询命中**唯一索引**（主键/唯一键） | 锁住确切一行 |
| **Gap Lock（间隙锁）** | 锁定索引记录**之间的间隙**（不锁记录本身） | 范围查询 或 等值查询**未命中** | 防止其他事务往间隙插入数据 → **防止幻读** |
| **Next-Key Lock（临键锁）** | **Record Lock + Gap Lock**（锁记录本身+前方间隙） | **RR级别下默认算法**（唯一索引等值命中时降级为Record） | 彻底解决幻读 |


### 经典案例分析（彻底吃透三种锁）

**数据**：`age` 普通索引，值为 `[10, 20, 30, 40]`。

| 场景 | SQL | 加的锁 | 锁范围（区间） |
| :--- | :--- | :--- | :--- |
| **唯一索引等值命中** | `WHERE id = 3`（主键） | **Record Lock** | 仅 `id=3` 这一行 |
| **普通索引等值命中** | `WHERE age = 20`（非唯一） | **Next-Key Lock + Gap Lock** | `(10, 20]` 和 `(20, 30)` → **锁住20前后间隙，防止插入19或21** |
| **等值查询未命中** | `WHERE age = 25`（不存在） | **Gap Lock** | 锁住 `(20, 30)` 这个间隙 |
| **范围查询** | `WHERE age > 20` | **Next-Key Lock** | 锁住 `(20, 30]`、`(30, 40]`、`(40, +∞)` |

**为什么场景2要锁前后间隙？**

因为新插入的记录可能排在`20`左边（如`19`），也可能排在右边（如`21`）。不锁前后间隙，事务执行期间别人插入新`20`，就会产生幻读。所以RR下**等值命中非唯一索引，必须锁住左开右闭 `(10, 20]` 以及后间隙 `(20, 30)`**。


### 隔离级别与锁的血缘关系

| 隔离级别 | 行锁算法 | 间隙锁 | 幻读 | 并发度 | 主从不一致风险 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **READ UNCOMMITTED** | Record Lock | ❌ | 脏读+幻读 | 最高 | 高 |
| **READ COMMITTED（RC）** | **Record Lock** | ❌ **禁用** | 幻读 | 较高 | **有（若binlog=STATEMENT）** |
| **REPEATABLE READ（RR）**（默认） | **Next-Key Lock** | ✅ 启用 | 无 | 中等 | 低 |
| **SERIALIZABLE** | 所有读加S锁 + Next-Key | ✅ | 无 | 最低 | 无 |

**关于RC主从不一致的原因（必背）**：
- RC允许幻读，意味着同一条SQL在不同时间点执行，返回的结果集可能不同。
- 若 `binlog_format = STATEMENT`（基于语句复制），从库重放SQL时筛选出的行可能与主库当初更新的行**不一致**，导致主从数据分叉。
- **解决方案**：生产环境必须将 `binlog_format` 设为 **`ROW`**（基于行复制），主库记录被改行的主键ID，从库精准修改，彻底杜绝歧义。
...
采用 **等待图（Wait-for Graph）** 算法，由后台线程实时检测：

1. **维护两张链表**：记录所有事务的**锁持有状态**和**锁等待状态**（T1在等哪把锁，被谁持有）。
2. **DFS深度优先遍历（环检测）**：每当新事务进入锁等待，检测线程即被触发，沿着“T1→T2→T3→T1”的等待关系遍历。一旦**发现回路（Cycle）**，立即判定为死锁。
3. **代价**：高并发下（如每秒上千锁请求），检测会消耗大量CPU。大厂有时会关闭死锁检测（`innodb_deadlock_detect = OFF`），依赖 `innodb_lock_wait_timeout` 超时回滚来换取CPU性能。

#### 死锁的四个必要条件（具象化理解）

| 必要条件 | 文字定义 | 通俗理解（两个程序员抢键盘鼠标） |
| :--- | :--- | :--- |
| **1. 互斥** | 资源同时只能被一个事务占用 | **键盘只有一个**，A拿走了B就拿不到。行锁天生互斥，这是死锁的前提土壤。 |
| **2. 持有并等待** | 持有资源的同时，还在等待其他资源 | A手里攥着键盘，**同时伸手去抢**B的鼠标。A并没有因为要鼠标就先放下键盘。 |
| **3. 不可抢占** | 已占有的资源不能被强行抢走 | B虽然急需键盘，但**不能从A手里硬抢**，只能等A主动释放。 |
| **4. 循环等待** | 等待关系形成一个闭环 | A在等B的鼠标，B也在等A的键盘 → 环路 `A→B→A`。若只是A等B、B等C（不闭环），那叫排队，不是死锁。 |

#### 实战避坑指南

| 引发场景 | 解决方案 |
| :--- | :--- |
| **批量更新顺序不一致**（T1先A后B，T2先B后A） | 业务层约定**按主键ID升序排序**再批量更新，打破循环等待。 |
| **RR + 间隙锁导致锁范围过大** | 降级为 **RC隔离级别** + `binlog_format=ROW`，减少间隙锁，降低死锁概率。 |
| **二级索引与主键锁顺序相反** | 尽量用**主键作为更新条件**，避免通过二级索引更新复杂事务。 |
| **大事务持有锁时间过长** | 拆分大事务，坚持**快进快出**，减少锁持有时间窗口。 |
...
当被问及MySQL锁时，最后这样总结，直接封神：

> “关于锁，我的核心原则是：**锁是保证数据一致性的手段，而非目的**。在实际开发中，我会优先通过**优化索引和SQL**来减少锁范围，而非依赖数据库加锁解决并发。
>
> 对于热点行更新（如秒杀库存），我倾向使用**乐观锁**，SQL写成：
> ```sql
> UPDATE stock SET count = count - 1 WHERE product_id = ? AND count > 0;
> ```
> 利用数据库行锁本身做原子CAS，配合重试机制，远比 `SELECT ... FOR UPDATE` 悲观锁吞吐量高。
>
> 此外，我会**严格监控长事务**，它是MDL阻塞和死锁的根源。代码中强制设置事务超时（`@Transactional(timeout = 3)`），并在压测时模拟高并发提前暴露死锁风险。”


### 高频面试自测清单（灵魂6问）

1. **InnoDB行锁到底锁什么？`UPDATE`不走索引会怎样？**  
   → 锁索引项。不走索引→锁全表（等价表锁），性能崩溃。

2. **RR级别下，唯一索引等值查询未命中记录，加什么锁？**  
   → **Gap Lock（间隙锁）**，锁住附近间隙，阻止插入。

3. **意向锁（IS/IX）什么时候会被阻塞？**  
   → 当有**表级X锁**（如 `LOCK TABLES t WRITE`）时，IS/IX会被阻塞。

4. **如何在不改代码的情况下降低死锁频率？**  
   → 隔离级别改为 `READ COMMITTED` + `binlog_format=ROW`，消除间隙锁。

5. **普通`SELECT`会加锁吗？**  
   → 不会（快照读，MVCC）。`SELECT ... FOR UPDATE` 才会加X锁。

6. **RC下为什么主从会不一致？**  
   → RC允许幻读，若binlog=STATEMENT，从库重放SQL时选中的行可能与主库不一致。必须用ROW格式复制。
...
![MySQL服务器逻辑架构图](../assets/images/high-performance-mysql/8ee1fac4-1ba9-47ad-81a0-d5ab0fc3f646.png)
...
MVCC（Multi-Version Concurrency Control，多版本并发控制）是行级锁的变种，是通过保存数据在某个时间点的快照来实现。实现方式典型的有乐观（optimistic）并发控制和悲观（pessimistic）并发控制。
...
![B-Tree索引](/assets/images/high-performance-mysql/44607b3d-14ab-42c6-9f84-48a56486cc9c.png)
...
![B-Tree索引举例](/assets/images/high-performance-mysql/10fbb641-ddd9-4c91-a73d-1dd16caaf14a.png)
...
![哈希索引结构](/assets/images/high-performance-mysql/b5da2883-ecb6-4b95-bb99-65fda6ff7a98.png)
...
![自适应哈希索引](/assets/images/high-performance-mysql/8d7b4aa1-013e-41e9-a81b-d5b4cc46c791.png)
...
![非聚簇索引](/assets/images/high-performance-mysql/04822719-a817-4bc2-9bc0-b6d3e3eb1dd5.JPG)
...
![区别](/assets/images/high-performance-mysql/bad2377f-c4cb-4a3c-968d-561c9b1a399b.PNG)

聚集索引的顺序插入：

![聚集索引的顺序插入](/assets/images/high-performance-mysql/b424e69d-ef4b-4d4c-8bd2-20bf5bc742d4.PNG)
...
![执行查询的过程](/assets/images/high-performance-mysql/754ea62d-9c04-46fe-ae7d-0ff353524861.png)
...
![MySQL复制如何工作](/assets/images/high-performance-mysql/200aa401-0173-44cc-a72a-3f811064ded5.png)

![将复制事件传递到更多的备库](/assets/images/high-performance-mysql/0af876da-0f16-4fbc-bc2a-fe76bf57e06f.png)
...
![复制过滤选项](/assets/images/high-performance-mysql/7558019f-1bd1-4262-ba69-da92a3e27776.png)

### 复制拓扑

#### 一主多备

![一主多备](/assets/images/high-performance-mysql/d904e0b1-8feb-48ef-8083-48d1c043b249.png)

#### 主动-主动模式下的主-主复制

![主-主复制](/assets/images/high-performance-mysql/85474e86-98b9-4f82-918c-baa5427f1d93.png)

#### 主动-被动模式下的主-主复制

![主动-被动模式下的主-主复制](/assets/images/high-performance-mysql/840aaf84-004c-4a29-a597-51971935160b.png)

#### 拥有备库的主-主结构

![拥有备库的主-主结构](/assets/images/high-performance-mysql/21e4dad7-4234-424a-ae7c-050943a067b2.png)

#### 环形复制

![环形复制拓扑](/assets/images/high-performance-mysql/d5e982cb-5885-41d8-9baa-73b8fd37918e.png)

#### 主库、分发主库以及备库

![一个主库一个分发主库和多个备库](/assets/images/high-performance-mysql/d387d5d6-eabb-4846-a7b0-a51bd8f35068.png)

#### 树或金字塔形

![金字塔形复制拓扑](/assets/images/high-performance-mysql/14a65b66-70ba-4151-b915-8af6ea9b2ad3.png)

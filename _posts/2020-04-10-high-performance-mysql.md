...
对于SELECT语句，在解析查询之前，服务器会先检查查询缓存4，如果能够在其中找到对应的查询，服务器就不必再执行查询解析、优化和执行的整个过程，而是直接返回查询缓存中的结果集。
...
“半双工”通信，一旦一端开始发送消息，另一端要接收完整个消息才能响应。
...
一条查询可以有很
多种执行方式，最后都返回相同的结果。MySQL使用基于成本的优化器，它将尝试预测一个查询使用某种执行计划时的成本，并选择其中成本最小的一个。

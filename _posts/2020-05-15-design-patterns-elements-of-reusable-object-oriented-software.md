...
![类适配器结构](/assets/images/design-patterns-elements-of-reusable-object-oriented-software/类适配器结构.png)

![对象适配器结构](/assets/images/design-patterns-elements-of-reusable-object-oriented-software/对象适配器结构.png)
...
使用Adapter模式时需要考虑的其他一些因素有:
  ...
  + 可插入的Adapter。当其他的类使用一个类时，如果所需的假定条件越少，这个类就更具可复用性。如果将接口匹配构建为一个类，就不需要假定对其他的类可见的是一个相同的接口。也就是说，接口匹配使得我们可以将自己的类加人到一些现有的系统中去，而这些系统对这个类的接口可能会有所不同。
...
![桥接模式结构图](/assets/images/design-patterns-elements-of-reusable-object-oriented-software/桥接模式结构图.png)
...
![典型的对象结构](/assets/images/design-patterns-elements-of-reusable-object-oriented-software/典型的对象结构.png)
...
![装饰模式结构图](/assets/images/design-patterns-elements-of-reusable-object-oriented-software/装饰模式结构图.png)
...
+ 比静态继承更灵活，可以用添加和分离的方法，用装饰在运行时刻增加和删除职责，减少类数量。且可以为一个特定的Component类提供多个不同的Decorator类，使得可以对一些职责进行混合匹配
+ 避免在层次结构高层的类有太多的特征。Decorator不试图在一个复杂的可定制的类中支持所有可预见的特征，相反可以定义一个简单的类，并且用Decorator类给它逐渐的添加功能，可以从简单的部件组合出复杂的功能。这样应用不比为不需要的特征付出代价，也更易于不依赖Decorator所扩展的类而独立地定义新类型的Decorator。
+ Decorator与它的Component不一样。Decorator是一个透明的包装，一个被装饰了的组件与这个组件是有差别的。
+ 有许多小对象。采用Decorator模式进行系统设计往往会产生许多看上去类似的小对象，这些对象仅仅在他们相互连接的方式上有所不同，而不是它们的类或是它们的属性值不同。这样会导致很容易进行定制，但是很难学习这个系统，排错也困难
...
![外观模式结构](/assets/images/design-patterns-elements-of-reusable-object-oriented-software/外观模式结构.png)
...
+ 它实现了子系统与客户之间的松耦合关系，而子系统内部的功能组件往往是紧耦合的。松耦合关系使得子系统的组件变化不会影响到它的客户。Facade模式有助于建立层次结构系统，也有助于对对象之间的依赖关系分层。Facade模式可以消除复杂的循环依赖关系。这一点在客户程序与子系统是分别实现的时候尤为重要。可以有效降低降低编译依赖性。
...
+ 使用Flyweight模式时，传输、查找和计算外部状态都会产生运行时的开销，尤其当flyweight原先被存储为内部状态时。然而，空间上的节省抵消了这些开销。共享的flyweight越多，空间节省也就越大。存储节约由以下几个因素决定:
  + 因为共享，实例总数减少的数目
  + 对象内部状态的平均数目
  + 外部状态是计算的还是存储的
...
![观察者模式结构](/assets/images/design-patterns-elements-of-reusable-object-oriented-software/观察者模式结构.png)
...
+ 创建目标到观察者之间的引用还是映射
+ 观察多个目标要观察者知道触发的是哪个目标
+ 水触发更新，目标主动通知、观察者主动轮询、使用者主动调用更新
+ 对已删除目标的悬挂引用，让他通知它的观察者对目标引用复位
+ 在发出通知前确保目标的状态自身是一致的

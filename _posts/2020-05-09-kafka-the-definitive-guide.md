...
- **物理存储层级**：`主题-分区（文件夹）` -> `Segment（文件组）` -> `.log（数据文件）` + `.index（偏移索引）` + `.timeindex（时间索引）`。
- **核心纠正**：Segment **不是**单个文件，也**不是**文件夹，而是**一组共享相同前缀的物理文件**。
- **传输单元 vs 存储单元**：
  - **RecordBatch（传输/写入单元）**：生产者发来时已打包好，大小通常几十KB~几MB。
  - **Segment（存储/清理单元）**：磁盘上的物理文件组，默认1GB，包含成千上万个 RecordBatch。
  - **关系**：1个 Segment = N个 RecordBatch 的顺序追加。


### 写入与生产者端机制

- **同步发送 vs 批量发送**：两者正交。`linger.ms` 决定攒批延迟（默认0），`future.get()` 决定业务线程是否阻塞。
- **高吞吐写入原理**：依赖 **顺序追加（Append-Only）** + **Page Cache**，写入时不走零拷贝。
- **消息可靠性**：`acks=all` + `min.insync.replicas`，配合幂等/事务实现 Exactly-Once。


### 消费者拉取机制（重点！拆解 `poll()` 过程）

- **拉取返回对象**：`ConsumerRecords`（按分区聚合的列表），物理本质是 `FetchResponse`（头部元数据 + 消息体）。
- **Broker 端“攒包”逻辑（由浅入深）**：
  1. **查索引定位**：通过 `.index` 二分查找，定位到目标 Offset 所在的物理位置（Position）。
  2. **向后扫描**：读取每个 RecordBatch 的**头部（仅4字节长度或几十字节元数据）**，累加总大小。
  3. **判断停止**：直到累计大小即将超过 `fetch.max.bytes`（默认50MB）或读完 HW（高水位）。
  4. **零拷贝发送**：拿着起始 Position 和累计总长度（Count），调用 `sendfile`。
- **原子性保证**：如果第一个 RecordBatch 超大（>1MB），Kafka 会打破限制，**完整返回这一个 Batch**，绝不切碎消息。


### 零拷贝与内存映射（碾压面试官的高阶区）

- **零拷贝（`sendfile`）发生在哪里**：**Broker 读取（消费/副本同步）环节**，绝对不在写入环节。
- **`sendfile` 如何截取部分数据**：通过 `offset`（起始字节）和 `count`（长度）两个参数，DMA 直接搬运，CPU 不参与数据拷贝。
- **扫描时的“半零拷贝”精妙设计**：
  - **头部元数据**（几十字节）：使用传统 `read` 读入 JVM 堆，用于逻辑判断（判断大小、CRC）。
  - **消息体（Payload）**（几MB）：**不动**，留在 Page Cache 中，直接由 DMA 发往网卡。
  - **代价计算**：扫描10个100KB的Batch，仅拷贝 `10*25字节=250字节` 到 JVM，省下1MB的拷贝开销。
- **`mmap`（内存映射文件）应用**：
  - 应用于 `.index` 索引文件（小文件、随机读）。
  - **为什么快**：砍掉了“内核态 -> 用户态”的数据拷贝，且消除频繁系统调用。
  - **为什么不用于 `.log`**：数据文件太大，占用虚拟地址空间，且顺序读场景下 `sendfile` 更高效（直发网卡）。


### 偏移量（Offset）管理机制

- **存储位置**：`__consumer_offsets` 内部主题（非 ZK）。
- **路由规则**：`hash(消费者组ID) % 50` 确定分区，该分区 Leader 即为 **Group Coordinator**。
- **提交与获取**：
  - **写入**：消费者发送 `OffsetCommitRequest`，Coordinator 追加写入该主题。
  - **读取**：Coordinator 直接返回**内存缓存**中的最新偏移量。
  - **清理**：启用 **Log Compaction（日志压缩）**，仅保留每个 Key（组+主题+分区）的最新偏移量（默认7天过期）。


### 元数据架构演进（KRaft vs ZooKeeper）

- **KRaft 集群角色**：**只有一个 Active Controller（主协调器）**，但它是从一组 **Controller Quorum（控制器仲裁）** 中选举出来的（一主多备）。
- **为什么 KRaft 比 ZK 快（即使都是单主）**：
  1. **写入链路**：KRaft 是**进程内本地追加**（一跳），ZK 是跨系统 RPC（两跳）。
  2. **批量写入**：KRaft 支持批量提交元数据，ZK 是单条事务（IOPS 爆炸）。
  3. **内存模型**：KRaft 元数据在 **Page Cache（堆外）**，ZK 在 **JVM 堆内**（导致 GC 瓶颈，分区数受限 20万）。
  4. **故障恢复**：KRaft 基于毫秒级心跳，恢复秒级；ZK 基于秒级超时，恢复分钟级。


### 存储清理与过期机制

- **未被消费的消息是否会删除**：**会**。Kafka 清理的最小单位是 **Segment（文件）**，不看消费进度。
- **触发条件**：
  - **大小限制**：`retention.bytes`（分区总大小）触发删除最旧 Segment。
  - **时间限制**：`retention.ms`（默认7天）触发删除。
- **特殊保护**：每个分区**至少保留一个 Segment**，即使该文件已过期，也不会被物理删除，保证最新数据可读。

## 快照和元数据流


### `__cluster_metadata` 主题（事件日志）

| 维度 | 核心要点 |
| :--- | :--- |
| **本质** | KRaft 的 **Raft 日志**，是集群状态的“真理之源”（Source of Truth）。 |
| **内容** | 存储所有元数据变更事件，如 Broker 注册、主题创建、分区 Leader 选举、配置变更、ISR 变动等。 |
| **物理特征** | - **单分区**（`__cluster_metadata-0`），保证全局有序。<br>- 存储于 Controller 节点（Broker 不存储，只拉取消费）。<br>- 使用 `snapshot` 清理策略，依赖快照来清理旧日志。 |
| **角色分工** | - **Active Controller**：唯一拥有写入权限的 Leader。<br>- **其他 Controller**：作为 Follower 同步数据，是“热备份”。<br>- **Broker**：作为 **Observer（观察者）**，不参与投票，主动拉取元数据更新并缓存本地。 |
| **空间占用** | 极小：每个分区约 40 字节元数据，10 万分区约 3.8 MB。预留 5 GB 磁盘足够。 |
| **清理策略** | - **大小保留**：默认 100 MB（`metadata.max.retention.bytes`）。<br>- **时间保留**：默认 7 天（`metadata.max.retention.ms`）。 |
| **核心优势** | - **全局有序**：单分区确保了变更的顺序性。<br>- **架构自包含**：移除了对 ZooKeeper 的外部依赖。<br>- **故障快速恢复**：新 Active Controller 元数据已在内存中，实现秒级故障转移。 |
...
| 维度 | 核心要点 |
| :--- | :--- |
| **本质** | 是 `__cluster_metadata` 日志在某个时刻的**全量状态“压缩包”**。 |
| **内容** | 记录了截至某个时间点的完整集群状态（如所有主题、分区、配置、Broker 列表等），是 `MetadataImage` 对象的序列化文件（`.checkpoint` 文件）。 |
| **生成触发条件** | - **数据量驱动**：自上次快照后新增日志达到阈值（`metadata.log.max.record.bytes.between.snapshots`，默认 **20 MB**）。<br>- **时间驱动**：距离上次快照超过阈值（`metadata.log.max.snapshot.interval.ms`，默认 **1 小时**）。<br>- **版本升级**：KRaft 协议版本变更时。 |
| **存储位置** | 磁盘上的独立文件（不同于日志文件），保存在 Controller 节点的 `meta.properties` 指定目录下。 |
| **自动管理** | 系统会保留**最新的若干个快照**，并自动清理旧的快照文件。 |
| **核心作用** | 1. **压缩日志**：防止 `__cluster_metadata` 日志无限增长。<br>2. **加速恢复**：节点重启时直接加载快照，再重放少量增量日志，恢复时间从分钟级降至秒级。 |


### 两者的协作关系（生成-清理-恢复）

它们通过一套闭环机制紧密配合，可以用“**图书馆的借阅记录总册 + 月度馆藏目录**”来形象理解：

| 阶段 | 运作方式 | 对应逻辑 |
| :--- | :--- | :--- |
| **生成** | 系统定期“阅读” `__cluster_metadata` 日志（流水账），将所有变更应用到内存状态机，再序列化为快照（状态摘要）写入磁盘。 | 类似每月初整理出一份馆藏目录。 |
| **清理** | 一旦新快照生成，该快照之前的**所有旧日志事件**即可被安全删除（遵循 `snapshot` 清理策略），磁盘空间得以释放。 | 有了新的月度目录，旧目录之前的每日借阅记录就可以归档清理。 |
| **恢复** | Controller/Broker 重启时，**先加载最新的快照**快速恢复大部分状态，然后**只重放快照之后的少量增量日志**，即可追平最新状态。 | 新管理员先看最新月度目录，再核对之后新增的少量记录。 |
...
| 参数 | 默认值 | 作用 |
| :--- | :--- | :--- |
| `metadata.log.max.record.bytes.between.snapshots` | 20 MB | 触发快照生成的增量日志大小阈值。 |
| `metadata.log.max.snapshot.interval.ms` | 3600000（1小时） | 触发快照生成的时间间隔。 |
| `metadata.max.retention.bytes` | 100 MB | 元数据日志（含快照）的总磁盘保留上限。 |
| `metadata.max.retention.ms` | 604800000（7天） | 元数据日志的保留时间。 |

## 常见问题与解决方案

### 消息积压

消费速度可以提升的情况：这种一般是突发流量或者消费突然恶化情况，可以通过增加消费速度解决，比如增加并发、找到消费瓶颈（慢SQL？链路上的限流超时？），进行提速或降级。
消费速度不可以提升，但是消费端可以停止发送：这种除了上面的情况，还多了一种上游异常发送了错误的消息，所以除了上述解决办法外，还可以选择清理、跳过消息。
消费速度不可以提升，同时消费端也不可停止、跳过：这种是从技术方案选型上就出了问题，没有考虑到消费跟不上生产的情况。只能这部分等待消费结束，同时重新设计技术方案。

### 消息重复

幂等是需要消费者支持的，可以通过幂等键、消费状态来保证重复消费幂等。
...
![包含多个分区的主题表⽰](/assets/images/kafka-the-definitive-guide/b7022229-6a90-4d5a-8a99-551634c32e50.png)
...
![消费者群组从主题读取消息](/assets/images/kafka-the-definitive-guide/761ba8b8-440a-4837-8c9a-48d024543f93.png)
...
![集群里的分区复制](/assets/images/kafka-the-definitive-guide/865cd464-1e80-4ccb-a94c-7eb4440d5dd6.png)
...
![⼀个简单的 Kafka 集群](/assets/images/kafka-the-definitive-guide/cbd05564-f9f2-4517-8473-edc78b47c811.png)

## 生产者

### 概览

![Kafka生产者组件图](/assets/images/kafka-the-definitive-guide/1dc4b5db-7a4d-4ab5-9088-b6e25e310173.png)
...
![1 个消费者收到 4 个分区的消息](/assets/images/kafka-the-definitive-guide/5e60a54a-3382-439b-af1f-5d4bf5371820.png)

如果消费者比topic下分区数少，那一个消费者可能会收到topic下多个分区的消息。

![2 个消费者收到 4 个分区的消息](/assets/images/kafka-the-definitive-guide/6df42659-f261-42ce-8ef2-7fedd0ad1115.png)

如果消费者和topic下分区数一致，那每个消费者都可以收到一个分区的消息。

![4 个消费者收到 4 个分区的消息](/assets/images/kafka-the-definitive-guide/9b230319-8c58-4aab-87d3-1990d3ca2a75.png)

如果消费者比topic下分区数多，那么将有消费者收不到分区消息。

![5 个消费者收到 4 个分区的消息](/assets/images/kafka-the-definitive-guide/09ac4cb8-dd05-4d01-8478-514fb7317674.png)

多个消费者组订阅同一topic，它们之间互不影响。

![两个消费者群组对应⼀个主题](/assets/images/kafka-the-definitive-guide/9f4b70e4-0100-484b-a8e7-80656ec0b14c.png)
...
![提交的偏移量⼩于客户端处理的最后⼀个消息的偏移量重复处理](/assets/images/kafka-the-definitive-guide/72249544-7f0b-45d9-8bff-645a52cb923e.png)

![提交的偏移量⼤于客户端处理的最后⼀个消息的偏移量跳过处理](/assets/images/kafka-the-definitive-guide/f9a985cb-ea43-49aa-b1c5-5ffe481214a0.png)
...
![Kafka处理请求](/assets/images/kafka-the-definitive-guide/70de761b-80ba-4312-a260-a2ac90feaa3c.png)

如果 broker 收到⼀个针对特定分区的请求，⽽该分区的⾸领在另⼀个 broker 上，那么发送请求的客户端会收到⼀个“⾮分区⾸领”的错误响应。然后客户端请求元数据。

![客户端路由](/assets/images/kafka-the-definitive-guide/a1bb2a8a-d191-496b-b90a-5c8874a9d4e1.png)
...
![客户端能收到的消息](/assets/images/kafka-the-definitive-guide/283ea0e4-1027-4553-9cea-c239f7e150f5.png)
...
![Hub和Spoke架构](/assets/images/kafka-the-definitive-guide/c2850715-14a8-47be-b78b-b936de9a4a4c.png)

![⼀个⾸领对应⼀个跟随者](/assets/images/kafka-the-definitive-guide/0d70d915-1fe1-4ec3-89e9-3ff886778c4b.png)

![双活架构](/assets/images/kafka-the-definitive-guide/912ccb14-8f41-4e10-864a-224a239b62a3.png)

![主备架构](/assets/images/kafka-the-definitive-guide/5555e53e-80a5-4974-a037-e3b0e90d8696.png)

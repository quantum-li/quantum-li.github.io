  ...
  */
  static int32_t max_array_length(BasicType type) {
    ...
    if ((size_t)max_jint < max_elements_per_size_t) {
      ...
      *  使用int类型来传递以字为单位表示的对象内存大小。所以需要避免数组内存大小加上header内存大小时int类型溢出。
  ...
  /* 这个方法只能被当做其他方法的入参调用，否则不会被进行常量折叠优化。
...
```
现在知道了Java中可以申请数组的最大长度是如何计算来的了，在实践中32位JVM为`Integer.MAX_VALUE-2`，64位中为`Integer.MAX_VALUE-3`。
...
是代表所有原始数据类型byte、double、float、int、long、short的对象类型（平台类型）的超类。其定义了平台类的数据类型转换方法。数据类型转换可能丢失精度，位宽。

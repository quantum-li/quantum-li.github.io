...
ClassLoader类使用委托模型来搜索类和资源。ClassLoader的每个实例都有一个关联的父类加载器。当请求查找类或资源时，ClassLoade实例会首先将类或资源的搜索委托给其父类加载器，如果父加载器不可以加载（无法加载或不负责此类），然后再尝试自己查找类或资源。虚拟机的内置类加载器，称为“引导类加载器（bootstrap class loader）”，它没有父级，但可以作为ClassLoader实例的父级。

...
[相关阅读：Class类详解](https://fandazeng.github.io/2019/06/12/java/Class%E7%B1%BB%E8%AF%A6%E8%A7%A3/)
...
Class包含了类的定义及获取类定义的一些方法，比如获取构造方法、获取成员方法、获取属性等。获取的构造方法一定是本类的，不会是父类中的构造方法。Class中带有`Declared`的方法都是获取本类的属性或方法。不带`Declared`的方法可以获取本类及父类中的所有**共有**的属性和方法。

```java
/**
使用给定的类加载器返回与具有给定字符串名称的类或接口关联的Class对象。给定类或接口的完全限定名称（与getName返回的格式相同），此方法尝试定位、加载和链接类或接口。指定的类加载器用于加载类或接口。如果参数加载器为空，则通过引导类加载器加载该类。仅当initialize参数为true且之前未初始化时，才会初始化该类。
如果name表示原始类型或void，将尝试在名称为name的未命名包中查找用户定义的类。因此，此方法不能用于获取任何表示原始类型或void的Class对象。如果name表示一个数组类，则数组类的组件类型被加载但不被初始化。
此方法会引发与Java语言规范中指定的加载、链接或初始化相关的错误。请注意，此方法不检查其调用者是否可以访问所请求的类。
...
/**
确定指定的实例是否与该Class兼容。此方法与instanceof等效。如果指定的实例为非null并且可以强制转换为此Class对象表示的引用类型而不引发ClassCastException，则该方法返回true。否则返回false。

当Class表示类，如果obj是Class或子类的实例则此方法返回true；
如果此Class表示一个数组类，如果obj可以通过恒等转换或扩展引用转换转换为数组类的对象，则此方法返回 true；
如果此Class对象表示一个接口，则如果obj的类或任何超类实现此接口，则此方法返回 true；
如果此Class表示原始类型，则此方法永远返回false（int.class.isInstance(2)==false）。
**/
public native boolean isInstance(Object obj);

/**
确定此Class表示的类或接口是否与参数表示的类或接口相同，或者是其超类或超接口。如果是，则返回 true；否则返回false。如果此Class对象表示原始类型，则如果指定的Class参数正是此Class对象，则此方法返回true；否则返回false。
具体来说，该方法测试参数表示的类型是否可以通过恒等转换或扩展引用转换转换为该Class对象表示的类型
**/
...
/**
返回此Class的全名。如果Class表示原始类型或void，则返回对应Java语言关键字。
...
/**
返回加载此Class的ClassLoader。如果由引导类加载器加载则返回null。
**/
...
/**
返回表示此Class所表示的类、接口、原始类型或void的超类。如果此Class表示Object类、接口、原始类型或void，则返回null。如果此对象表示数组类，则返回Object类。
**/
...
如果超接口是参数化类型，则为其返回的[Type](https://www.jianshu.com/p/a84e485f8077)对象必须准确反映源代码中使用的实际类型参数。如果之前没有创建过代表每个超接口的参数化类型，则创建它。有关参数化类型的创建过程的语义，请参见 ParameterizedType 的声明。
如果此对象表示一个类，则返回值是一个数组，其中包含表示该类实现的所有接口的对象。数组中接口对象的顺序对应于该对象所代表的类的声明的implements子句中接口名称的顺序。在数组类的情况下，接口Cloneable和Serializable按该顺序返回。
如果此对象表示一个接口，则该数组包含表示该接口直接扩展的所有接口的对象。数组中接口对象的顺序对应于该对象表示的接口声明的extends子句中接口名称的顺序。
如果此对象表示一个没有实现接口的类或接口，则该方法返回一个长度为0的数组。
...
如果此对象表示一个类，则返回值是一个数组，其中包含表示该类实现的所有接口的对象。数组中接口对象的顺序对应于该对象所代表的类的声明的implements子句中接口名称的顺序。
...
如果超接口是参数化类型，则为其返回的[Type](https://www.jianshu.com/p/a84e485f8077)对象必须准确反映源代码中使用的实际类型参数。如果之前没有创建过代表每个超接口的参数化类型，则创建它。有关参数化类型的创建过程的语义，请参见ParameterizedType的声明。
如果此对象表示一个类，则返回值是一个数组，其中包含表示该类实现的所有接口的对象。数组中接口对象的顺序对应于该对象所代表的类的声明的implements子句中接口名称的顺序。在数组类的情况下，接口Cloneable和Serializable按该顺序返回。
如果此对象表示一个接口，则该数组包含表示该接口直接扩展的所有接口的对象。数组中接口对象的顺序对应于该对象表示的接口声明的extends子句中接口名称的顺序。
如果此对象表示一个没有实现接口的类或接口，则该方法返回一个长度为0的数组。
如果此对象表示原始类型或void，则该方法返回长度为0的数组。
...
/**
返回此类或接口的Java语言修饰符，以整数编码。修饰符由public、protected、private、final、static、abstract和interface常量组成；它们应该使用类修饰符的方法进行解码。
...
/**
如果类或接口是另一个类的成员（内部类），则返回表示在其中声明它的类的Class对象。
...
/**
返回此Class中定义的所有类和接口。这包括类声明的公共、受保护、默认（包）访问以及私有类和接口，但不包括继承的类和接口。
...
/**
返回此类及所有父类中的pulic内部类
如果此Class表示原始类型、数组类或void，则此方法还返回长度为0的数组。
**/
...
/**
返回此Class表示的类声明的所有构造函数。这些是公共的、受保护的、默认（包）访问和私有构造函数。
...
/**
返回此Class表示的类或接口的指定属性。不包括继承来的属性。
如果这个Class代表一个数组类型，那么这个方法无法获取length属性。
**/
public Field getDeclaredField(String name){...}

/**
返回此类及其所有超类中定义的指定public属性。
此方法**无法找到**数组类型中的length属性。
**/
public Field getField(String name){...}

/**
返回此Class表示的类或接口声明的所有字段。这包括公共、受保护、默认（包）访问和私有字段，但不包括继承的字段。
...
/**
返回此类或接口及所有父类中的pulic属性
如果此Class表示数组类型、原始类型或void，则此方法返回长度为0的数组。
返回数组中的元素没有排序，也没有任何特定的顺序。
**/
public Field[] getFields(){...}

/**
返回此Class表示的类或接口的指定声明方法。 不包括继承来的方法
...
/**
返回此类或接口及其所有超类中指定的public方法。
如果名称是“<init>”或“<clinit>”，则会引发 NoSuchMethodException。
如果多个具有相同参数类型的方法（Java不允许，但JVM允许），并且其中一个方法的返回类型比其他任何方法都更具体，则返回该方法；否则任意选择其中一种方法。
如果是数组类型无法找到clone()方法
在此表示的类或接口的超接口中声明的静态方法不被视为类或接口的成员。
**/
public Method getMethod(String name, Class<?>... parameterTypes){...}

/**
返回此Class表示的类或接口的所有声明方法，包括公共、受保护、默认（包）访问和私有方法，但不包括继承方法。
如果此Class表示具有多个已声明的具有相同名称和参数类型但返回类型不同的方法的类型，则返回的数组对于每个此类方法都有一个Method对象。
如果此Class表示具有类初始化方法 <clinit> 的类型，则返回的数组没有对应的Method对象。
...
/**
返回此类或接口及所有父类中的public方法
如果此Class表示具有多个具有相同名称和参数类型但返回类型不同的公共方法的类型，则返回的数组对于每个此类方法都有一个Method对象。
如果此Class表示具有类初始化方法 <clinit> 的类型，则返回的数组**没有**对应的Method对象。
如果此Class表示一个数组类型，则返回的数组具有一个Method对象，用于该数组类型从Object继承的每个公共方法。但它不包含clone。
如果此Class表示一个接口，则返回的数组不包含任何来自Object的隐式声明的方法。因此，如果在此接口或其任何超接口中没有显式声明任何方法，则返回的数组的长度为0。（请注意，表示类的Class始终具有从Object继承的公共方法。）
如果此Class表示原始类型或void，则返回的数组的长度为0。
在此Class表示的类或接口的超接口中声明的静态方法**不被视为**类或接口的成员。
返回数组中的元素没有排序，也没有任何特定的顺序。
**/

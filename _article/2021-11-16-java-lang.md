...
一个字符类型Character，该类提供了大量的静态方法用于确定字符的类别（小写字母、数字等）以及字符大小写转换。Character类的字段和方法是根据来自Unicode标准的字符信息定义的，特别是作为Unicode字符数据库一部分的UnicodeData文件。该文件可从[Unicode Consortium](http://www.unicode.org)。

...
HashMap 中的 hash 操作是对元素的 hash 值与 容量-1 进行按位与计算，容量是2的倍数。
...
可以看出来，只有 `oldCap` 二进制中唯一的一个 1 的位置计算结果发生了变化。所以 HashMap 中的 rehash 操作使用的是 `hash & oldCap` 是否等于 0，等效与重新进行 hash 计算。等于 0 位置没有变化，不等于 0 位置移动到高位，而高位位置就是 `n + oldCap` 和重新 hash 的结果是一样的。这样的好处是可以减少一步 hash 操作时对于容量减 1 的步骤。

#### 节点是 Node 类型，数据结构是链表

当前节点是 Node 类型，说明在扩容之前的链表元素少于 6 个，所以即使扩容后进行 rshash ，桶中元素数量只会更少。所以直接把 rshash 后拆分的两条链表移动到相应位置就完成了。
...
当前节点是 TreeNode 类型，说明在扩容之前的链表元素大于等于 6 个，拆分后的两条链表就会有两种情况，大小可以少于 6 个也可以大于等于 6 个。因此根据拆分后的大小情况进行 `treeify`  和 `untreeify` 操作。

...
HashMap的实例有两个影响其性能的参数：初始容量和扩容因子。容量是哈希表中的桶数，初始容量只是哈希表创建时的容量。扩容因子触发扩容的阈值。当哈希表中的 ⚠️ 条目数超过扩容因子和当前容量的乘积时，对哈希表进行重新哈希（即重建内部数据结构），扩容至大约两倍（极限是1<<30）的桶数。

默认扩容因子 (0.75) 在时间和空间成本之间提供了良好的折衷。较高的值会减少空间开销，但会增加查找成本（反映在HashMap类的大多数操作中，包括get和put ）。在设置其初始容量时，应考虑映射中的预期条目数及其扩容因子，以尽量减少重新哈希操作的次数。如果初始容量大于最大条目数除以扩容因子，则不会发生重新哈希操作。
...
提供了4个构造函数进行初始化，但是初始化只是设置了扩容阈值 `threshold` 和扩容因子 `loadFactor`，真正的申请内存创建表延迟到了扩容方法中。初始化方法虽然提供了初始容量参数 `initialCapacity` ，但是该初始容量并没有被直接使用，而是通过 `tableSizeFor` 方法计算了大于等于给定初始容量的 2 的倍数的一个容量，并赋值到了 `threshold` 用于第一次初始化容量。

### 容量为什么一定要是2的倍数

如果用取余（这种情况取余和取模是[一样的](https://www.zhihu.com/question/30526656/answer/160437482)）的方法计算元素的位置，取余操作会使用累加器，HashMap 作为 JDK 功能使用频率极高，任何一点性能上的损失都会被无限放大。因此选用了轻量级的位运算来进行模运算，这个时候就需要容量是 2 的倍数才能方便计算。计算方法是把容量减一当作掩码进行按位与操作: `(n - 1) & hash` 来获取元素的位置。例如：
...
3. 非第一次扩容，如果容量已经到了最大容量，就把阈值也设置为最大容量。否则最大容量扩充一倍（左移一位）。如果容量小于`DEFAULT_INITIAL_CAPACITY`，新阈值就通过容量乘扩容因子计算而得；但是如果容量大于等于`DEFAULT_INITIAL_CAPACITY`，新阈值就会通过位移来翻倍。原因是扩容因子是个浮点数，当容量太小的时候，翻倍操作会扩大误差。比如当容量为2扩容因子为0.75时，此时阈值等于$2 \times 0.75 = 2$，当容量翻倍时阈值等于$4 \times 0.75 = 3$，如果阈值通过翻倍计算就会是4，和3的误差相对容量来说较大。
